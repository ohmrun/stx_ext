...
@:callable abstract Unfold<T,R>(T->Iterable<R>) from T->Iterable<R>{
  @:from static public function fromFunction<T,R>(fn:T -> Option<Tuple<T, R>>):Unfold<T,R>{
    return unfold.bind(_,fn);
  }
  public inline function new( v : T->Iterable<R> ) {
    this = v;
  }
  @:noUsing static inline function unfold<T, R>(initial: T, unfolder: T -> Option<Tuple<T, R>>): Iterable<R> {
    return {
